...
if (isset($_POST['upload_prewed'])) {
    $uploaded_images = array();
    $altertxt = $_POST['alttxt'];
    foreach ($_FILES['prewed_gallery']['name'] as $key => $val) {
        $upload_dir = "prewedding_gallery/img/";
        $upload_file = $upload_dir . $_FILES['prewed_gallery']['name'][$key];
        $filename = $_FILES['prewed_gallery']['name'][$key];
        if (move_uploaded_file($_FILES['prewed_gallery']['tmp_name'][$key], $upload_file)) {
            $uploaded_images[] = $upload_file;
            $insert_sql = "INSERT INTO `tbl_prewed_gallery` (`prewed_imgpath`, `prewed_imgalt`) VALUES('prewedding_gallery/img/" . $filename . "', '" . $altertxt . "')";
...
<head>
    ...
    <title>Admin - Pre-Wedding Editing</title>
...
<body style="overflow-x: hidden;">
    ...
    <div class="wrapper">
        ...
        <div id="content">
            ...
            <!-- Main Content -->
            <h2>Pre-Wedding Portfolio Edting</h2>
                ...
                <div class="col-lg-4 bg-light rounded px-4">
                    ...
                    <form action="" method="post" enctype="multipart/form-data">
                        <div class="form-group">
                            ...
                            <input type="file" name="prewed_gallery[]" class="form-control p-1" required multiple>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <input type="submit" name="upload_prewed" class="btn btn-warning btn-block" value="upload image">
            ...
            <div style="width: 80vw;overflow-x:auto; word-wrap: break-word;">
                ...
                <tr>
                    ...
                    $query = "SELECT * FROM `tbl_prewed_gallery`";
                    $result = mysqli_query($con, $query);
                    $srno = 0;
                    while ($row = mysqli_fetch_assoc($result)) {
                        $srno = $srno + 1;
                        $img_id = $row['prewed_id'];
                        echo '
                <tbody>
                <tr>
                    <td>' . $srno . '</td>
                    <td>' . $row['prewed_imgpath'] . '</td>
                    <td>' . $row['prewed_imgalt'] . '</td>
                    <td><img src="' . $row['prewed_imgpath'] . '" width="400px" height = "150px"></td>
                    <td>
                        <a href="prewedding_gallery/upd_prewed.php?upd_id=' . $img_id . '" class="edit btn btn-sm btn-primary" id="' . $row['prewed_id'] . '">Edit</a>
                        <a href="prewedding_gallery/del_prewed.php?del_id=' . $row['prewed_id'] . '" class="delete btn btn-sm btn-primary" id="' . $row['prewed_id'] . '">Delete</a>
                    </td>
                </tr>
                </tbody>
                ';
            ...
            <div class="">
                <div class="resizable">
                    <iframe src="../prewedding.php" height="200" width="300" title="Iframe_wedding"></iframe>
                </div>
            </div>
