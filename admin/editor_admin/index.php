...
<body id="editor" ng-app="ImageEditor" ng-strict-di>
    ...
    <div ng-controller="MainController" ng-cloak style="height: 100%">
        ...
        $imgid = $_GET['upd_id'];
        $cat = $_GET['cat'];
        /*echo '<script type="application/ng-template" id="texture-upload-dialog-template">
        ...
        if (empty($imgid)) {
            ...
            switch ($cat) {
                case "wedding":
                    $result = mysqli_query($con, "SELECT * FROM `tbl_wed_gallery` WHERE `wed_id` = $imgid");
                    $data = mysqli_fetch_array($result);
                    break;
                case "prewedding":
                    $result = mysqli_query($con, "SELECT * FROM `tbl_prewed_gallery` WHERE `prewed_id` = $imgid");
                    $data = mysqli_fetch_array($result);
                    break;
                case "baby":
                    $result = mysqli_query($con, "SELECT * FROM `tbl_baby_gallery` WHERE `baby_id` = $imgid");
                    $data = mysqli_fetch_array($result);
                    break;
                case "model":
                    $result = mysqli_query($con, "SELECT * FROM `tbl_model_gallery` WHERE `model_id` = $imgid");
                    $data = mysqli_fetch_array($result);
                    break;
                default:
        ...
        <script type="application/ng-template" id="save-image-dialog">
            <md-dialog class="upload-file-dialog save-dialog">
                    ...
                    <md-radio-group ng-model="imageType">
                        ...
                        <md-radio-button value="json">Edittable</md-radio-button>
                    </md-radio-group>
                    ...
                    <p ng-if="imageType === 'json'">This will download a file with current pixie editor state so you can load it into editor later.</p>
            ...
            <md-dialog class="upload-file-dialog">
                    <div ng-show="openImageMode === 'open'">
                        ...
                        <div class="buttons" ng-show="!canOpenImage">
                            ...
                            <md-button ng-click="openSampleImage()">Same Image</md-button>
                        </div>
